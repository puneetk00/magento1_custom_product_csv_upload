...
class Mage_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Abstract
{
    ...
    protected $_imagesArrayKeys = array(
        '_media_image', 'image', 'small_image', 'thumbnail', 'back_image'
    );
    ...
    public function validateRow(array $rowData, $rowNum)
    {
		...
		/* image file exists check for media import start */
		// _media_image check
		$_smallimage = Mage::getBaseDir('media') . DS . 'import' . $rowData['_media_image'];
		if(isset($rowData['_media_image'])){
			if(! file_exists($_smallimage)){
				$this->addRowError(Mage::helper('importexport')->__("Image not found for the column '_media_image'"), $rowNum);
			}
		}
		// back_image check
		$_smallimage = Mage::getBaseDir('media') . DS . 'import' . $rowData['back_image'];
		if(isset($rowData['back_image'])){
			if(! file_exists($_smallimage)){
				$this->addRowError(Mage::helper('importexport')->__("Image not found for the column 'back_image'"), $rowNum);
			}
		}
		// small_image check
		$_smallimage = Mage::getBaseDir('media') . DS . 'import' . $rowData['small_image'];
		if(isset($rowData['small_image'])){
			if(! file_exists($_smallimage)){
				$this->addRowError(Mage::helper('importexport')->__("Image not found for the column 'small_image'"), $rowNum);
			}
		}		
		// thumbnail check
		$_thumbnailimage = Mage::getBaseDir('media') . DS . 'import' . $rowData['thumbnail'];
		if(isset($rowData['thumbnail'])){
			if(! file_exists($_thumbnailimage)){
				$this->addRowError(Mage::helper('importexport')->__("Image not found for the column 'thumbnail'"), $rowNum);
			}
		}
		// image check
		$_image = Mage::getBaseDir('media') . DS . 'import' . $rowData['image'];
		if(isset($rowData['image'])){
			if(! file_exists($_image)){
				$this->addRowError(Mage::helper('importexport')->__("Image not found for the column 'image'"), $rowNum);
			}
		}
		/* image file exists check for media import start */
